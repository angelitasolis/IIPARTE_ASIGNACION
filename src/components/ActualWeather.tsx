...
function ActualWeather({ weather }: ChildProps) {
  ...
  return (
    <>
      ...
      <h4 className='mt-3 py-5 text-2xl font-bold'>{weather?.current.last_updated}</h4>
      {
        weather?.location && (
                ...
                <div className='flex flex-col justify-center items-center px-10 text-center'>
                  {
                    weather?.current.condition.icon && (
                      <img src={"https:" + weather?.current.condition.icon ?? ''} alt="icon current condition" width={50} height={50} />
                    )
                  }
                  <h2 className='text-3xl font-bold'>{weather?.current.temp_c +'°C.'}</h2>
